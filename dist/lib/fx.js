...
exports.normalizeToMarketCurrency = normalizeToMarketCurrency;
// Default map is intentionally sparse; provide your own via env FX_RATES_USD (JSON)
const DEFAULT_FX_RATES_USD = {
    USD: 1,
    EUR: 1.08,
    GBP: 1.27,
    JPY: 0.0065,
...
function convertAmount(amount, fromCode, toCode, rates) {
    ...
    if (outAsGBX)
        to = 'GBP';
    if (from === to)
        return outAsGBX ? adjAmount * 100 : adjAmount;
...
function formatAmount(amount, code, maximumFractionDigits = 2) {
    try {
...
function normalizeToMarketCurrency(amount, code) {
